def incremental_sip_return(principal,fund_growth_rate,duration_in_year,rate_of_increase_principal):
    ...
    if duration_in_year == 0:
        ...
        new_pricipal = principal*(1+(rate_of_increase_principal/100))
        duration_in_month -= 1
...
def inflation_cost(current_amount,inflation_rate,duration_in_year):
    ...
    return principal*((1+(inflation_rate/100)))**duration_in_year
...
fund_return_amount = incremental_sip_return(principal,fund_growth_rate,duration_in_year,rate_of_increase_principal)
debt_return_amount = incremental_sip_return(principal,debt_rate,duration_in_year,rate_of_increase_principal)

annuity_amount_on_fund = annuity_per_month(fund_return_amount,debt_rate)  
annuity_amount_on_debt = annuity_per_month(debt_return_amount,debt_rate)

required_total_monthly = inflation_cost(current_amount,inflation_rate,duration_in_year)

gap_to_be_fulfilled_for_equity = annuity_amount_on_fund - required_total_monthly
    
gap_to_be_fulfilled_for_debt =  annuity_amount_on_debt - required_total_monthly

invested_amount = principal*12*duration_in_year
